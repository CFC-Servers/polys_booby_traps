...
if SERVER then
    function ENT:Initialize()
        ...
        --print( constraint.HasConstraints( self ) )
...
function ENT:SETUP()
    if SERVER then
        if not constraint.FindConstraint( self, "Rope" ) then
...
function ENT:Think()
    ...
    if not self.deployed and self.UseTimer < CurTime() then
        self.deployed = true
        self:SETUP()
    end

    if not IsValid( self.ropey ) and not self.broken and self.primed then
        self.broken = true
    ...
    if IsValid( self.ropey ) and not self.broken and self.primed and self.primed2 then
        local trace = {}
...
function ENT:Use( user )
    if user:IsPlayer() and self.UseTimer < CurTime() then
        ...
        if not constraint.FindConstraint( self, "Rope" ).Ent2:GetModel() == "models/props_c17/trappropeller_lever.mdl" then
            self.stick = ents.Create( "prop_physics" )
            self.stick:SetPos( self:GetPos() + Vector( 0, 0, 10 ) )
            self.stick:SetModel( "models/props_c17/TrapPropeller_Lever.mdl" )
            self.stick.Owner = self
