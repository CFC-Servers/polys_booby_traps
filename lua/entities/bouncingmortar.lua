...
if SERVER then
    function ENT:Initialize()
        ...
        self.fuse:ManipulateBoneScale( 0, Vector( 1, 1, 1 ) * 0.4 )
        self.fuse:PhysicsInitBox( Vector( -1, -1, -1 ), Vector( 1, 1, 1 ) )
        self.fuse:SetCollisionGroup( COLLISION_GROUP_DEBRIS )
        self.fuse:SetCollisionBounds( Vector( -0.01 - 0.01, -0.01 ), Vector( 0.01, 0.01, 0.01 ) )
...
function ENT:Explode()
    if self.primed and not self.Exploding then
        self.primed = false
