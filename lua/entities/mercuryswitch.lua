...
function ENT:CutWire()
	if not self.target.broken and self.Primed == 2 then
...
function ENT:Think()
	if not IsValid( self.ropey ) and not self.broken and self.target and self.Primed == 2 then
		self.broken = true
		ED = EffectData()
...
function ENT:Touch( entity )
	if not IsValid( self.ropey ) then
		...
		if user:IsPlayer() and IsValid( self.ropey ) then
			if self.Primed ~= 2 then
				...
				if not self.Primed then
					...
					timer.Create( "MercurySwitchTimer", 1, 4, function()
						if IsValid( self ) then
							self.Primed = self.Primed + 0.25
						end
					end )
				end
			end
